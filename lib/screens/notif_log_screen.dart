...
// ─────────────────────── NOTIF EVENT LOGGER ───────────────────────
/// Lightweight event logger that persists to SharedPreferences.
/// Works across isolates (background handler) because it reads/writes
/// SharedPreferences on every call.
class NotifEventLog {
  /// Log a notification lifecycle event.
  /// [event] = short tag: 'onMessage', 'onBackground', 'onOpenedApp',
  ///           'getInitialMessage', 'saveResult', 'willPresent',
  ///           'didReceiveRemoteNotif', 'localNotifShown', 'error'
  static Future<void> log(
    ...
    try {
      ...
      await p.reload(); // pick up writes from background isolate
  ...
  static String get _platformStr {
    // Use dart:io Platform - import it at top of file
    try {
...
class NotifLogScreen extends StatefulWidget {
  ...
  @override State<NotifLogScreen> createState() => _NotifLogScreenState();
}
...
class _NotifLogScreenState extends State<NotifLogScreen> {
  ...
  static const Color kLight   = Color(0xFFCDEBD6);
  ...
  Color _eventColor(String event) {
    ...
    if (event == 'onMessage' || event == 'onBackground' ||
        event == 'onOpenedApp' || event == 'getInitialMessage' ||
        event == 'didReceiveRemoteNotif' || event == 'willPresent') return kPrimary;
    return kMuted;
  }
  ...
  IconData _eventIcon(String event) {
    switch (event) {
      ...
      case 'willPresent': return Icons.notifications_active_rounded;
      case 'saveResult': return Icons.save_rounded;
      case 'localNotifShown': return Icons.notifications_rounded;
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Notification Event Log', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 17)),
        elevation: 0,
        actions: [
          IconButton(onPressed: _load, icon: const Icon(Icons.refresh_rounded, size: 22)),
          if (_logs.isNotEmpty)
            TextButton(
              onPressed: () async {
              ...
              child: const Text('Clear', style: TextStyle(color: kRed, fontSize: 13, fontWeight: FontWeight.w600)),
            ),
        ],
      ),
      body: _loading
          ...
          : _logs.isEmpty
              ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  const Text('No events logged yet', style: TextStyle(color: kMuted, fontSize: 15, fontWeight: FontWeight.w600)),
                      ...
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(children: [
                            ...
                            const Spacer(),
                            Text(_timeStr(e['ts'] as String? ?? ''), style: const TextStyle(fontSize: 10, color: kMuted)),
                          ]),
                          if ((e['title'] as String? ?? '').isNotEmpty) ...[
                            const SizedBox(height: 4),
                            Text('title: ${e['title']}', style: const TextStyle(fontSize: 12, color: kText)),
                          ],
                          ...
                          if (e['saved'] != null) ...[
                            const SizedBox(height: 2),
                            Text('saved: ${e['saved']}', style: TextStyle(
                              fontSize: 12, fontWeight: FontWeight.w700,
                              color: e['saved'] == true ? kGreen : kOrange)),
                          ],
                          if ((e['error'] as String? ?? '').isNotEmpty) ...[
                            const SizedBox(height: 2),
                            Text('error: ${e['error']}', style: const TextStyle(fontSize: 11, color: kRed)),
                          ],
                          if ((e['imageUrl'] as String? ?? '').isNotEmpty) ...[
                            const SizedBox(height: 2),
                            Text('image: yes', style: const TextStyle(fontSize: 10, color: kMuted)),
                          ],
                          if ((e['type'] as String? ?? '').isNotEmpty) ...[
                            const SizedBox(height: 2),
                            Text('type: ${e['type']}', style: const TextStyle(fontSize: 10, color: kMuted)),
                          ],
                        ]),
                      ]),
                    );
